...
def summarize(data, pred_list):
    ...
    for i in data:
        temp = {}
        moods = ['joy','sadness','fear','anger','disgust','surprise','anticipation','acceptance']
        key = list(i.keys())[0];
        total = len(i[key])
        temp_percent = 99 #should change before production to 100
        if total != 0: 
            temp['joy'] = format((i[key].count(1) / total) * temp_percent, '.2f')
            temp['sadness'] = format((i[key].count(2) / total) * temp_percent, '.2f')
            temp['fear'] = format((i[key].count(3) / total) * temp_percent, '.2f')
            temp['anger'] = format((i[key].count(4) / total) * temp_percent, '.2f')
            temp['disgust'] = format((i[key].count(5) / total) * temp_percent, '.2f')
            temp['surprise'] = format((i[key].count(6) / total) * temp_percent, '.2f')
            temp['anticipation'] = format((i[key].count(7) / total) * temp_percent, '.2f')
            temp['acceptance'] = format((i[key].count(8) / total) * temp_percent, '.2f')
        else:
...
def find_summarize_max_emo(summarize_data):
    ...
    key_list.remove('zone')
    key_list.remove('name')
    key_list.remove('name_id')
    key_list.remove('place_id')
