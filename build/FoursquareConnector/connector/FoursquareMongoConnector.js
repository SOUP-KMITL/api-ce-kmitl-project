...
"4b30d634f964a520fbfb24e3",
"4b584060f964a520a44f28e3"]
...
function saveFQcheckin (venueID){
	...
	console.log('save checkin by ' + venueID)
        const x = JSON.parse(res)   
        if(x.meta.code == 200)    
        {
            ...
            checkin.createAt = ""+new Date().getTime()	
...
const cronSave1min = new cronJob('*/1 * * * *', () => {
    db.place2.find({},{'venue_id':1}).forEach(function(err, docs){        
	if (docs != null)
        saveFQcheckin(docs['venue_id'])
        if(err){
            console.log(err)
        }
